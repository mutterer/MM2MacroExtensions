	...
	public class MM2_Recorder {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (recording) {
						...
						for (int i = 0; i < logLines.length; i++) {
							...
							if (lineInstant.isAfter(recordingStart)) translateLine(l);							
						}
						Editor ed = (Editor)IJ.runPlugIn("ij.plugin.frame.Editor", "");
						ed.createMacro("RecordedMM.ijm", recorded);

						studio.core().enableDebugLog(false);
		...
		public void addLine(String s) {
			...
			if (l.contains("Did snap image from current camera")) {
				// the snap image case
				// 2019-08-22T10:16:42.545600 tid0x70000114a000 [dbg,Core] Did snap image from current camera
				addLine("Ext.snap();");
			} else if (l.contains("Did set camera Camera exposure to")) {
				// the exposure case
				// 2019-08-22T10:15:45.098246 tid0x700001d77000 [dbg,Core] Did set camera Camera exposure to 100.000 ms
				...
				// 2019-08-22T10:15:45.088301 tid0x700001d77000 [dbg,Core] Config group Channel: will apply preset FITC
				String[] p = Tools.split(l, " ,:");
				...
				// 2019-08-22T10:15:45.099798 tid0x700001d77000 [dbg,Core:dev:Camera] Did set property "Mode" to "Noise"
				String[] p = Tools.split(l, ":]\"");
				addLine((groupOpened?" // ":"")+"Ext.setDeviceProperty('" + p[4] + "','" + p[6] + "','" + p[8] + "');");

			} else if (l.contains("Will start relative move ")) {
				// the set device property case
				// 2019-08-22T10:16:42.394513 tid0x70000114a000 [dbg,Core] Will start relative move of Z by offset 0.00000 um
				// 2019-08-22T10:16:42.394617 tid0x70000114a000 [dbg,Core] Will start relative move of XY by (10.000, 15.000) um
				String dz = "0";
				String dx = "0";
				String dy = "0";
				...
				// the set device property case
				// 2019-08-22T10:16:42.404090 tid0x70000114a000 [dbg,Core] Will start absolute move of Z to position 0.00000 um
				// 2019-08-22T10:16:42.404151 tid0x70000114a000 [dbg,Core] Will start absolute move of XY to position (0.000, 0.000) um
				String dz = "0";
				String dx = "0";
				String dy = "0";
				if (l.contains("Z")) {
